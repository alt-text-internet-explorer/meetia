...
export default async function Page({ oId }) {
  let collections = []

  try {
    await connectDB()

    collections = await getAllCollectionsFromOwner(oId)
  } catch (e) {
  ...
  // let collection = collections.getReviewsFromUID("6a0dead69b6a94b09b4cea0d")
